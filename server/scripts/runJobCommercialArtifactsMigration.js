...
try {
  await pool.query(await fs.readFile(path.join(scriptDir, '12_job_commercial_artifacts.sql'), 'utf8'));
  console.log('Job commercial artifacts migration applied successfully.');
} finally {
  await pool.end();
}
