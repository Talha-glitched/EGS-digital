...
try {
  await pool.query(await fs.readFile(path.join(scriptDir, '33_inventory_refactor_photos_quantity_notes_status.sql'), 'utf8'));
  console.log('Inventory refactor migration (photos, quantity, notes, status) applied successfully.');
} finally {
  await pool.end();
}
