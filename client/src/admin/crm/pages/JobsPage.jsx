...
export default function JobsPage() {
  ...
  const fetchJobs = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res = await crmApiFetch(`/api/admin/sales/jobs?${params.toString()}`);
      setJobs(res.items || []);
  ...
  function handleOpenEdit(job) {
    ...
    setFormData({
      ...
      currentStatus: isStatusKnown ? job.currentStatus : (job.currentStatus ? 'Other' : 'Inquiry'),
  ...
  async function handleSaveJob(e) {
    ...
    try {
      ...
      if (editingJob) {
        await crmApiFetch(`/api/admin/sales/jobs/${editingJob._id}`, {
          method: 'PATCH',
          body: JSON.stringify(payloadToSave),
        });
      } else {
        await crmApiFetch('/api/admin/sales/jobs', {
          method: 'POST',
          body: JSON.stringify(payloadToSave),
        });
      }
  ...
  async function handleDeleteJob(id) {
    ...
    try {
      await crmApiFetch(`/api/admin/sales/jobs/${id}`, { method: 'DELETE' });
      fetchJobs(pagination.page);
    } catch (err) {
    ...
    <div className="crm-page-shell space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-[var(--color-line)]">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-[var(--color-ink)] flex items-center gap-2">
            <Briefcase className="h-5.5 w-5.5 text-brand shrink-0" /> Jobs Directory
          </h1>
          <p className="text-xs text-[var(--color-ink-muted)] mt-0.5">
            Past and active production jobs repository. Automatically synced from closed Sales Pipeline opportunities.
          </p>
        </div>
      ...
      <div className="crm-metric-grid cols-4">
        <div className="crm-card p-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-[var(--color-ink-muted)]">Total Jobs</p>
      ...
      <div className="crm-card overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500 text-xs">No job records found matching filters.</div>
        ) : (
          ...
          <div className="crm-card w-full max-w-2xl overflow-hidden shadow-xl border border-slate-200 bg-white">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-line)] bg-slate-50/50">
              <h3 className="text-base font-bold text-[var(--color-ink)] flex items-center gap-2">
                ...
                {editingJob ? `Edit Job #${editingJob.jobNo}` : 'Add New Job'}
              </h3>
              <button
                type="button"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Sticker printing with lamination..."
                    className="crm-input"
                  />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Job Review / Notes</label>
                  <textarea
                    rows={2}
                    value={formData.jobReview}
                    onChange={(e) => setFormData({ ...formData, jobReview: e.target.value })}
                    placeholder="Client feedback, follow-up notes..."
                    className="crm-input"
                  />
                </div>
              </div>
