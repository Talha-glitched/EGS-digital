      ...
      {!form ? <LoadingState label="Loading task…" /> : <div className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(280px,0.75fr)]">
        ...
        <aside className="space-y-4">
          <section className="rounded-xl border border-neutral-200 bg-white p-4">
            ...
            <div className="mt-4 space-y-2">{(record?.evidence || []).map((item) => <div key={item.id} className="rounded-lg bg-neutral-50 p-2 text-xs"><div className="flex items-center justify-between gap-2"><strong>{item.fileName || 'Evidence note'}</strong><span className="text-[10px] text-neutral-400">{new Date(item.createdAt).toLocaleDateString('en-AE')}</span></div>{item.note && <p className="mt-1 text-neutral-600">{item.note}</p>}{item.url && <a className="mt-1 inline-block text-brand hover:underline" href={item.url} target="_blank" rel="noreferrer">Open file</a>}</div>)}{!record?.evidence?.length && <p className="text-xs text-neutral-400">No evidence added.</p>}</div>
          </section>
          <section className="rounded-xl border border-neutral-200 bg-white p-4">
