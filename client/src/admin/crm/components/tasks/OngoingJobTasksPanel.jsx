...
export default function OngoingJobTasksPanel({
  ongoingJobId,
  opportunityId,
  companyId,
  ongoingJobOwner,
  ...
  const targetOwner = ongoingJobOwner || opportunityOwner;
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await crmApiFetch(
        `/api/admin/sales/tasks?status=${encodeURIComponent(statusFilter)}&ongoingJobId=${encodeURIComponent(targetId)}`,
      );
      setTasks(data.items || []);
    } catch (err) {
  ...
  useEffect(() => {
    loadOwnerOptions(visibleTasks, [targetOwner])
      .then(setOwnerOptions)
      .catch(() => setOwnerOptions(buildOwnerOptions(visibleTasks, [targetOwner])));
  }, [visibleTasks, targetOwner]);
  ...
  return (
    <div className="crm-opp-tasks-panel">
      ...
      {!visibleTasks.length ? (
        <EmptyState
          icon={CalendarCheck2}
          title={statusFilter === 'Done' ? 'No completed tasks' : 'No tasks for this Ongoing Job'}
