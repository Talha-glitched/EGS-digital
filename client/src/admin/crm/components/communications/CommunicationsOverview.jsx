    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-2xs text-neutral-400">{when(item.occurredAt || item.lastMessageAt)}</span>
        </div>
      </div>
      <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-neutral-600">{item.preview || 'No message preview available.'}</p>
    ...
    <div className="border-b border-[var(--color-line)] px-4 py-3 last:border-b-0">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <button type="button" onClick={() => onOpen({ conversationId: item.conversationId, messageId: item.messageId })} className="min-w-0 flex-1 text-left">
          ...
          <p className="mt-0.5 truncate text-2xs text-neutral-500">{[item.personName, item.companyName, item.campaignName].filter(Boolean).join(' · ') || 'Direct communication'}</p>
          <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-neutral-600">{item.preview || 'No message preview available.'}</p>
        </button>
        <div className="flex shrink-0 flex-col items-end gap-2">
          <span className="text-2xs text-neutral-400">{when(item.occurredAt)}</span>
          <select aria-label="Review owner" className="crm-select !w-36 !py-1 text-2xs" value={item.ownerUserId || ''} disabled={assigning} onChange={(event) => onAssign(item, event.target.value)}>
            <option value="">Unassigned</option>
            {owners.map((owner) => <option key={owner.id} value={owner.id}>{owner.name}</option>)}
          </select>
          <button type="button" className="crm-btn-primary !px-2.5 !py-1.5 text-xs" onClick={() => onReview(item)}><CheckCircle2 className="h-3.5 w-3.5" />Review</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <section className="crm-card overflow-hidden">
        <div className="border-b border-[var(--color-line)] px-4 py-3">
          ...
          <p className="mt-0.5 text-2xs text-neutral-500">Search subject, email text, contact, company or campaign. Results always open the immutable source conversation.</p>
      ...
      <section className="crm-card overflow-hidden">
        <div className="flex items-center justify-between gap-3 border-b border-[var(--color-line)] px-4 py-3">
          <div><h2 className="text-xs font-semibold">Replies awaiting human review</h2><p className="mt-0.5 text-2xs text-neutral-500">AI does not classify these; a person owns the decision.</p></div>
  ...
  return (
    <div className="crm-card overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-line)] px-4 py-3">
        <div><h2 className="text-xs font-semibold">Communication linked to operational work</h2><p className="mt-0.5 text-2xs text-neutral-500">One source conversation can support more than one Job without copying its email body.</p></div>
