...
function DecisionComposer({ artifactType, versionId, contacts, options, busy, onSave }) {
  ...
  return (
    <div className="mt-3 grid gap-2 rounded-lg bg-neutral-50 p-2.5 sm:grid-cols-[150px_180px_minmax(0,1fr)_auto]">
      <select className="crm-select text-xs" value={decision} onChange={(e) => setDecision(e.target.value)}>{options.map((value) => <option key={value} value={value}>{DECISION_LABELS[value]}</option>)}</select>
      <select className="crm-select text-xs" value={personId} onChange={(e) => setPersonId(e.target.value)}><option value="">Client contact optional</option>{contacts.map((contact) => <option key={contact._id} value={contact._id}>{contact.name || contact.email}</option>)}</select>
      <input className="crm-input text-xs" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Evidence or comment, e.g. approved by email" />
    ...
    <div className="rounded-lg border border-neutral-200 p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-2xs text-neutral-500">{version.author} · {when(version.createdAt)}{version.totalAmount != null ? ` · ${formatCurrency(version.totalAmount)}` : ''}</p>
          {version.revisionNote && <p className="mt-2 text-xs leading-5 text-neutral-700">{version.revisionNote}</p>}
        </div>
...
export default function JobArtifactsPanel({ ongoingJobId, contacts = [], active = true }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-brand/20 bg-brand-soft/30 p-3 text-xs leading-5 text-neutral-600"><strong>Independent approvals:</strong> design and commercial acceptance are recorded separately against the exact file version. Adding a revision never overwrites an earlier file or decision.</div>
