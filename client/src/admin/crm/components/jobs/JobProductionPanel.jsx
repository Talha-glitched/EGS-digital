...
function UpdateComposer({ busy, onSave }) {
  ...
  return <form className="mt-3 grid gap-2 sm:grid-cols-[130px_minmax(0,1fr)_auto_auto]" onSubmit={async (e) => { e.preventDefault(); if (!note.trim() && !file) return; await onSave({ type, note, file }); setNote(''); setFile(null); e.currentTarget.reset(); }}>
    <select className="crm-select text-xs" value={type} onChange={(e) => setType(e.target.value)}>{Object.entries(UPDATE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
    <input className="crm-input text-xs" value={note} onChange={(e) => setNote(e.target.value)} placeholder="What happened, what is blocked, or what was completed?" />
...
export default function JobProductionPanel({ ongoingJobId, active = true }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><ClipboardCheck className="h-4 w-4 text-brand" /><h3 className="text-sm font-semibold">Production readiness</h3></div><p className="mt-1 text-xs text-neutral-500">Advisory checks make risk visible without preventing an authorized EGS decision.</p></div><Badge tone={data.readiness.ready ? 'success' : 'warning'}>{data.readiness.ready ? 'Core ready' : 'Core information missing'}</Badge></div>
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{data.readiness.checks.map((check) => <div key={check.key} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs ${check.ok ? 'bg-emerald-50 text-emerald-800' : 'bg-amber-50 text-amber-800'}`}>{check.ok ? <CheckCircle2 className="h-3.5 w-3.5" /> : <AlertTriangle className="h-3.5 w-3.5" />}{check.label}</div>)}</div>
      {data.readiness.warnings.length > 0 && <p className="mt-3 text-xs font-medium text-amber-700">Visible exceptions: {data.readiness.warnings.join(' · ')}</p>}
    </section>
    ...
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      ...
      {activeRelease && <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-900"><strong>Active release:</strong> {activeRelease.releaseBasis === 'approved' ? 'Approved basis' : 'Authorized exception'} · {when(activeRelease.releasedAt)} by {activeRelease.releasedBy}{activeRelease.poPending ? ' · PO pending' : ''}{activeRelease.depositPending ? ' · Deposit pending' : ''}{activeRelease.note ? <p className="mt-1">{activeRelease.note}</p> : null}<div className="mt-3 flex gap-2"><input className="crm-input min-w-0 flex-1 bg-white! text-xs" value={revocationReason} onChange={(e) => setRevocationReason(e.target.value)} placeholder="Reason to stop this production basis" /><button type="button" className="crm-btn-secondary text-rose-700" disabled={busy || !revocationReason.trim()} onClick={revokeRelease}>Revoke</button></div></div>}
      <form className="mt-4 space-y-3" onSubmit={createRelease}>
        <div><p className="mb-2 text-xs font-semibold text-neutral-700">Exact design version(s)</p><div className="flex flex-wrap gap-2">{data.designs.map((design) => <label key={design.id} className="flex items-center gap-2 rounded-lg border border-neutral-200 px-2.5 py-2 text-xs"><input type="checkbox" checked={release.designVersionIds.includes(design.id)} onChange={(e) => setRelease((v) => ({ ...v, designVersionIds: e.target.checked ? [...v.designVersionIds, design.id] : v.designVersionIds.filter((id) => id !== design.id) }))} /><span>{design.seriesTitle} V{design.version}</span><Badge tone={design.latestDecision === 'approved' ? 'success' : 'neutral'}>{design.latestDecision || 'No decision'}</Badge></label>)}</div></div>
        ...
        <div className="flex flex-wrap gap-4 text-xs text-neutral-600"><label className="flex items-center gap-2"><input type="checkbox" checked={release.poPending} onChange={(e) => setRelease((v) => ({ ...v, poPending: e.target.checked }))} />PO pending</label><label className="flex items-center gap-2"><input type="checkbox" checked={release.depositPending} onChange={(e) => setRelease((v) => ({ ...v, depositPending: e.target.checked }))} />Deposit pending</label></div>
    ...
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      ...
      <form className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4" onSubmit={createActivity}>
        ...
        <label className="text-2xs text-neutral-500">Start<input type="datetime-local" className="crm-input mt-1 text-xs" value={activity.plannedStart} onChange={(e) => setActivity((v) => ({ ...v, plannedStart: e.target.value }))} /></label><label className="text-2xs text-neutral-500">End<input type="datetime-local" className="crm-input mt-1 text-xs" value={activity.plannedEnd} onChange={(e) => setActivity((v) => ({ ...v, plannedEnd: e.target.value }))} /></label>
      ...
      {!data.activities.length ? <div className="mt-4"><EmptyState title="No production activities" description="Plan real operational work here; keep small reminders in Tasks." /></div> : <div className="mt-5 space-y-3">{data.activities.map((item) => <div key={item.id} className="rounded-xl border border-neutral-200 p-3">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold">{item.title}</p><Badge tone={item.status === 'completed' ? 'success' : item.status === 'blocked' ? 'warning' : 'info'}>{STATUS_LABELS[item.status]}</Badge></div><p className="mt-1 text-2xs text-neutral-500">{TYPE_LABELS[item.activityType]} · {item.ownerName || 'Unassigned'} · {when(item.plannedStart)} → {when(item.plannedEnd)}{item.locationName ? ` · ${item.locationName}` : ''} · actual {(Number(item.actualMinutes || 0) / 60).toFixed(1)} h</p>{item.blocker && <p className="mt-2 flex items-center gap-1 text-xs font-medium text-amber-700"><FileWarning className="h-3.5 w-3.5" />{item.blocker}</p>}</div><div className="flex gap-2"><select className="crm-select text-xs" value={item.status} onChange={(e) => updateActivity(item.id, { status: e.target.value })}>{data.activityStatuses.map((value) => <option key={value} value={value}>{STATUS_LABELS[value]}</option>)}</select><button className="crm-btn-ghost p-2! text-rose-600" onClick={() => archiveActivity(item.id)}><Trash2 className="h-3.5 w-3.5" /></button></div></div>
        {item.updates.length > 0 && <div className="mt-3 space-y-1 border-t border-neutral-100 pt-2">{item.updates.slice(0, 5).map((update) => <p key={update.id} className="text-2xs text-neutral-500"><strong>{UPDATE_LABELS[update.type]}</strong> · {update.author} · {when(update.createdAt)}{update.note ? ` — ${update.note}` : ''}{update.url ? <> · <a className="text-brand" href={update.url} target="_blank" rel="noreferrer">{update.fileName}</a></> : null}</p>)}</div>}
