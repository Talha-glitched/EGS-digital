  ...
  return (
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      <div className="mb-4 flex items-start gap-3">
        ...
        <div><h3 className="text-sm font-semibold text-neutral-900">{title}</h3><p className="mt-0.5 text-[11px] leading-5 text-neutral-500">{description}</p></div>
      </div>
      {children}
    </section>
  );
...
function ProgressSelect({ value, onChange, values }) {
  return (
    <select className="crm-select min-w-32 text-[11px]" value={value || 'not_started'} onChange={(event) => onChange(event.target.value)}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand/20 bg-brand-soft/30 px-4 py-3">
        <p className="max-w-2xl text-[11px] leading-5 text-neutral-600">Start with only what is known. A Job can contain several services, phases, and locations; detail can be added as the design and quotation mature.</p>
      ...
      <Section icon={Boxes} title="Work packages" description="The distinct pieces EGS must design, quote, make, source, or install.">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-[10px] text-neutral-500"><span>{data.services.length} service categories available</span><span>{data.uoms.length} units of measure available</span></div>
            ...
            {data.workPackages.map((item) => (
              <div key={item.id} className="rounded-lg border border-neutral-200 p-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0"><p className="text-xs font-semibold text-neutral-900">{item.title}</p><div className="mt-1 flex flex-wrap gap-1.5 text-[10px] text-neutral-500">{item.serviceLabel && <Badge tone="info">{item.serviceLabel}</Badge>}{item.phaseId && <span>{phaseMap[item.phaseId]}</span>}{item.locationId && <span>· {locationMap[item.locationId]}</span>}</div></div>
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  <select className="crm-select text-[11px]" value={item.serviceOfferingId || ''} onChange={(event) => update('work-packages', item.id, { serviceOfferingId: event.target.value })}><option value="">Service (optional)</option>{data.services.map((service) => <option key={service.id} value={service.id}>{service.label}</option>)}</select>
                  <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] gap-2">
                    <input type="number" min="0" step="any" className="crm-input text-[11px]" defaultValue={item.quantity ?? ''} placeholder="Qty" onBlur={(event) => update('work-packages', item.id, { quantity: event.target.value })} />
                    <select className="crm-select text-[11px]" value={item.uomId || ''} onChange={(event) => update('work-packages', item.id, { uomId: event.target.value })}><option value="">UOM</option>{data.uoms.map((uom) => <option key={uom.id} value={uom.id}>{uom.label}</option>)}</select>
                  </div>
                  <select className="crm-select text-[11px]" value={item.scopeState || 'draft'} onChange={(event) => update('work-packages', item.id, { scopeState: event.target.value })}>{data.scopeStates.map((state) => <option key={state} value={state}>{SCOPE_LABELS[state] || state}</option>)}</select>
                  <select className="crm-select text-[11px]" value={item.phaseId || ''} onChange={(event) => update('work-packages', item.id, { phaseId: event.target.value })}><option value="">Phase (optional)</option>{data.phases.map((phase) => <option key={phase.id} value={phase.id}>{phase.name}</option>)}</select>
                  <select className="crm-select text-[11px]" value={item.locationId || ''} onChange={(event) => update('work-packages', item.id, { locationId: event.target.value })}><option value="">Location (optional)</option>{data.locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select>
                  <input type="date" className="crm-input text-[11px]" value={item.targetDate ? String(item.targetDate).slice(0, 10) : ''} onChange={(event) => update('work-packages', item.id, { targetDate: event.target.value })} aria-label="Work package target date" />
                </div>
              </div>
            ))}
          </div>
