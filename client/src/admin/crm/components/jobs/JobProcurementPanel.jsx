...
function UpdateComposer({ busy, onSave }) {
  ...
  return <form className="mt-3 flex flex-wrap gap-2" onSubmit={async (event) => { event.preventDefault(); await onSave({ type, note }); setNote(''); }}>
    <select className="crm-select w-36 text-xs" value={type} onChange={(e) => setType(e.target.value)}>{Object.entries(UPDATE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
    <input className="crm-input min-w-56 flex-1 text-xs" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Delivery update, issue, resolution or cost explanation" required />
...
export default function JobProcurementPanel({ ongoingJobId, active = true }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      ...
      <p className="mt-1 text-xs text-neutral-500">A supplier is a company identity EGS can reuse across Jobs. Add only capability labels useful for searching.</p>
      ...
      {data.suppliers.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{data.suppliers.map((item) => <span key={item.id} className="rounded-full border border-neutral-200 px-2.5 py-1 text-2xs text-neutral-600"><strong>{item.name}</strong>{item.capabilityTags?.length ? ` · ${item.capabilityTags.join(', ')}` : ''}</span>)}</div>}
    </section>
    ...
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      ...
      <p className="mt-1 text-xs text-neutral-500">Use this when comparing suppliers. Skip it and create a direct commitment for ordinary purchasing.</p>
      <form className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(e) => { e.preventDefault(); submit('/rfqs', rfq, () => setRfq({ title: '', workPackageId: '', requiredBy: '', requirement: '' })); }}>
        ...
        <label className="text-2xs text-neutral-500">Required by<input type="date" className="crm-input mt-1 text-xs" value={rfq.requiredBy} onChange={(e) => setRfq((v) => ({ ...v, requiredBy: e.target.value }))} /></label>
      ...
      {!data.rfqs.length ? <div className="mt-4"><EmptyState title="No supplier comparisons" description="Create an RFQ only when comparing prices or terms will help the Job." /></div> : <div className="mt-4 space-y-3">{data.rfqs.map((item) => <div key={item.id} className="rounded-xl border border-neutral-200 p-3">
        <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-semibold">{item.title}</p><p className="mt-1 text-2xs text-neutral-500">{item.workPackageTitle || 'Whole Job'} · required {item.requiredBy || 'date not set'}</p>{item.requirement && <p className="mt-2 text-xs text-neutral-600">{item.requirement}</p>}</div><select className="crm-select text-xs" value={item.status} onChange={(e) => patch(`/rfqs/${item.id}`, { status: e.target.value })}>{data.rfqStatuses.map((status) => <option key={status} value={status}>{status[0].toUpperCase() + status.slice(1)}</option>)}</select></div>
        {item.quotes.length > 0 && <div className="mt-3 overflow-x-auto"><table className="w-full text-left text-xs"><thead className="text-neutral-400"><tr><th className="pb-1">Supplier</th><th className="pb-1">Price</th><th className="pb-1">Lead time</th><th className="pb-1">Status</th><th /></tr></thead><tbody>{item.quotes.map((quote) => <tr key={quote.id} className="border-t border-neutral-100"><td className="py-2 font-medium">{quote.supplierName}</td><td>{amount(quote.amount, quote.currency)}</td><td>{quote.leadTimeDays == null ? '—' : `${quote.leadTimeDays} days`}</td><td><Badge tone={quote.status === 'accepted' ? 'success' : 'neutral'}>{quote.status}</Badge></td><td className="text-right"><button type="button" className="text-2xs font-semibold text-brand" onClick={() => setCommitment((v) => ({ ...v, supplierId: quote.supplierId, supplierQuoteId: quote.id, workPackageId: item.workPackageId || '', description: item.title, committedAmount: quote.amount }))}>Use quote</button></td></tr>)}</tbody></table></div>}
    ...
    <section className="rounded-xl border border-neutral-200 bg-white p-4">
      ...
      <p className="mt-1 text-xs text-neutral-500">This is the cost and delivery promise EGS is relying on. It can come from a formal quote or a direct supplier agreement.</p>
      <form className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(e) => { e.preventDefault(); submit('/commitments', commitment, () => setCommitment({ supplierId: '', supplierQuoteId: '', workPackageId: '', reference: '', description: '', committedAmount: '', expectedDeliveryAt: '' })); }}>
        ...
        <label className="text-2xs text-neutral-500">Expected delivery<input type="datetime-local" className="crm-input mt-1 text-xs" value={commitment.expectedDeliveryAt} onChange={(e) => setCommitment((v) => ({ ...v, expectedDeliveryAt: e.target.value }))} /></label>
        ...
        {commitment.supplierQuoteId && <p className="text-2xs text-emerald-700 lg:col-span-4">Linked to selected supplier quote. Saving will mark it accepted.</p>}
      </form>
      {!data.commitments.length ? <div className="mt-4"><EmptyState title="No supplier commitments" description="Record an agreed outsourced item, material order or subcontracted service here." /></div> : <div className="mt-5 space-y-3">{data.commitments.map((item) => <div key={item.id} className="rounded-xl border border-neutral-200 p-3">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold">{item.supplierName} · {item.description}</p><Badge tone={item.status === 'delivered' ? 'success' : item.status === 'cancelled' ? 'neutral' : 'info'}>{STATUS_LABELS[item.status]}</Badge></div><p className="mt-1 text-2xs text-neutral-500">{item.workPackageTitle || 'Whole Job'} · committed {amount(item.committedAmount, item.currency)} · actual {item.actualAmount == null ? 'not recorded' : amount(item.actualAmount, item.currency)} · expected {when(item.expectedDeliveryAt)}</p></div><div className="flex flex-wrap gap-2"><select className="crm-select text-xs" value={item.status} onChange={(e) => patch(`/commitments/${item.id}`, { status: e.target.value })}>{data.commitmentStatuses.map((status) => <option key={status} value={status}>{STATUS_LABELS[status]}</option>)}</select><input type="number" min="0" step="0.01" className="crm-input w-32 text-xs" defaultValue={item.actualAmount ?? ''} placeholder="Actual AED" onBlur={(e) => { if (String(item.actualAmount ?? '') !== e.target.value) patch(`/commitments/${item.id}`, { actualAmount: e.target.value }); }} /></div></div>
        {item.updates.length > 0 && <div className="mt-3 space-y-1 border-t border-neutral-100 pt-2">{item.updates.slice(0, 6).map((update) => <p key={update.id} className={`text-2xs ${update.type === 'issue' ? 'font-medium text-amber-700' : 'text-neutral-500'}`}><strong>{UPDATE_LABELS[update.type]}</strong> · {update.author} · {when(update.createdAt)} — {update.note}</p>)}</div>}
